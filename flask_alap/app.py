...
def calc():
    ...
    suly = float(suly_str)
    if suly < 0:
        return render_template("calc.html", boxes=boxes_data, message={"text": "Súly nem lehet negatív!", "category": "danger"})
    ...
    if not item:
        return render_template("calc.html", boxes=boxes_data, message={"text": "Cikkszám nem található", "category":"danger"})
        
    print(cikkszam, suly, file = sys.stderr)
    print(type(cikkszam), type(suly), file = sys.stderr)
    if box_suly:=request.args.get('box_suly'):
